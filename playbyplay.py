...
from summary import playclass, playstartclass
from playResult import playtextclass
from possession import possessionclass, playpossessionclass
...
class playbyplay(espn, output):
    ...
    def getTeamGameData(self, gameID, seasonData, teamMetaData):
        teamSeasonData = seasonData.teams.get(teamMetaData["ID"])
            ...
            for gameIdent,gameData in yearData.items():
                ...
                for idr,drive in enumerate(driveData):
                    ...
                    for ipl,drivePlay in enumerate(drivePlays):
                        ...
                        startVals = self.parsePlayStart(playStartText, playNo, len(drivePlays), drivePlay, debug=verydebug)
